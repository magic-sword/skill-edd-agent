...
新規のスキルやエージェントワークフローを開発・配備する場合、出力先の絶対パス（例: `/workspace/src/agents/...`）をプログラム内にハードコードしたり、ファイルを事後に無理に移動するような処理ステップを追加してはなりません。

*   **論理配置キー（`target_entry`）の利用**:
    新規開発を行う際は、必ず開発ワークフローの `target_entry` 引数に、プロジェクトの `skills.json` で定義済みの論理名（例: `tool` や `agent`）を指定し、`SkillsState` を通じて配置先ディレクトリを決定論的に自動解決させてください。
*   **物理パスの伝播**:
