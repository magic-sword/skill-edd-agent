...
- **Google ADK 2.0 / MCP アダプタ (`adk` / `mcp`)**: `create_adk_skill_toolset`, `EddSkillToolset` (コアスキルプリロード & `EddSkillRegistry` 動的オンデマンド解決による真の Progressive Disclosure, ADK公式 `UnsafeLocalCodeExecutor` 標準注入、裏口ハック全廃・決定論的スクリプト実行), `EddSkillRegistry`, `create_mcp_server`
