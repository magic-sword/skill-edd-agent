...
* **Google ADK 2.0 純正スクリプト実行基盤への一本化と環境パリティ (`SkillPackage.execute_script`)**:
  自前の一時展開スクリプト生成コードやプライベート属性（`_tools`）への裏口アクセス、手製 subprocess 直叩きを完全排除。Google ADK 2.0 純正のスクリプト実行エンジン（`_SkillScriptCodeExecutor` / `LocalSubprocessCodeExecutor` 等の `BaseCodeExecutor`）に一本化。リソース（`references/`, `assets/`, `scripts/`）の安全な一時展開、パストラバーサル防御、公式引数展開（`short_options` ➔ `args` ➔ `positional_args`）を保証し、契約テスト（`ContractTestRunner`）とエージェント本番実行（`RunSkillScriptTool`）の完全な環境パリティを実現。
