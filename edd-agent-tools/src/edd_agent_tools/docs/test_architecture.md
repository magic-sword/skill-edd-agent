...
## 2. テスト共通スキーマと決定論的判定 (Evaluation Schemas)

テストアセットのポータビリティと再現性を確保するため、テストデータは `edd-agent-tools` で一元管理された以下の2つの主要スキーマフォーマットを使用します。

### ① 決定論的 Black-box CLI 契約テスト: `EvalCaseSet` (`*_contract.evalset.json`)
スキルのスクリプト（`scripts/`）をサンドボックス隔離環境下で決定論的にサブプロセス実行し、CLI引数、終了コード、標準出力・標準エラー出力を検証するためのスキーマ。
*   **特徴**:
    - **Black-box 実行**: Python 内部コードをインポートせず、CLI インターフェース（`cli_args`）経由でサブプロセス実行。
    - **完全決定論的検証**: `expected_exit_code`（終了コード）および `expected_stdout_contains`（出力キーワード）による厳密なアサーションを行い、ハルシネーションや環境依存を物理的に排除。

### ② 軌跡・シミュレーション評価テスト: `SimulationEvalSet` / `TrajectoryEvalSet`
Google ADK 公式および Anthropic 標準の軌跡シミュレーションに準拠した、ユーザープロンプトに対するトリガー精度やツールの呼び出し順序を検証するためのスキーマ。
*   **特徴**:
    - **トリガー判定 (`trigger`)**: ユーザーの入力発話に対し、対象スキルが正しく選択されるか（正例）／対象外のプロンプトで誤発火しないか（負例）をアサーション。
    - **行動軌跡 (`trajectory`)**: ワークフローにおいて、期待されるツール・スクリプトの呼び出しシーケンス（`tool_uses`）を適切に辿っているかを検証。
