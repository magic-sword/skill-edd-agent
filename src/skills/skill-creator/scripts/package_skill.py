#!/usr/bin/env python3
"""
Skill Packager CLI Wrapper (Thin Convention-Based Client)

Anthropic / Google ADK 2.0 準拠のスキルZIPパッケージングを edd 統合 CLI に委譲します。
外部ライブラリへの直接 import 依存を持たず、プロセス境界（CLI-as-an-API）で動作します。

Usage:
    package_skill.py <skill-path> [--output <path>]
"""
...
def main():
    ...
    cmd = base_cmd + ["package"] + sys.argv[1:]
