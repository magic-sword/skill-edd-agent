...
3. **【第 3 段階】決定論的な組み立て**
   * ジェネレータが、第1段階で生成されたロジックコードと、第2段階のマッピング JSON を、ADK 規約に基づく `workflow.py` テンプレートの中にはめ込み、 edges 接続を含めて 100% 決定論的に組み立てて出力します。
...
## 7. 依存スキルの決定論的自動マッピングとフォールバック

他スキル呼び出しノードを決定論的に自動生成する際、設計書（`design.json`）の `inputs` 引数マッピングの記述漏れによる Pydantic `ValidationError` を防ぐため、**設計情報の動的解析と自動フォールバックマッピング**を実装しています。

1. **依存スキルの入力パラメータロード**:
   * 生成器（`workflow_generator.py`）は、対象ステップが依存するスキルの `design.json` を動的にロードし、必要とされている入力パラメータ（`parameters`）のリストをスキャンします。
2. **自動マッピングの補完**:
   * 設計書の `inputs` に明示的なパラメータマッピングが書かれていない場合であっても、自動的に `tool_context.state.get("<param_name>")` から同名の値を取得して `Input` に渡す代入コードを自動補完して生成します。
   * これにより、開発中のスキーマ変更や設計書の記述漏れがあっても、型エラーをコンパイル段階で未然に防ぐことができます。
