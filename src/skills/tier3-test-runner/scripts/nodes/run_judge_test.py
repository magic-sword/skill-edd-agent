...
def run_judge_test(tool_context: ToolContext) -> str:
    state = SkillsState()
    module = state.get_skill("judge-test-executor").load_module()
    
    skill_name = tool_context.state.get("skill_name") or tool_context.state.get("skill")
    eval_set_path = tool_context.state.get("judge_eval_set_path") or tool_context.state.get("eval_set_path")
    ...
    tool_context.state["judge_test_status"] = status
