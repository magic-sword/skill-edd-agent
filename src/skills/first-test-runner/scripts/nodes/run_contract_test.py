...
def run_run_contract_test_step(tool_context: ToolContext) -> str:
    ...
    eval_file = os.path.join(base_path, f"{skill_name.replace('-', '_')}_unit.evalset.json")
    ...
    print(f"DEBUG run_contract_test: type(res)={type(res)}, res={res}")
    is_success = False
    ...
    tool_context.state["contract_test_is_success"] = is_success
