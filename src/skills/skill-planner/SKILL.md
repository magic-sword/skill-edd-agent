---
name: skill-planner
description: 要件プロンプトを分析し、単体スキル(skill)、ワークフロー(workflow)、または事前スキル開発提案(proposal)へ分類して計画立案するスキル。
---

# skill-planner

## 概要
ユーザーから提供された要件プロンプトを分析し、開発の実現方法・難易度に応じて適切な開発ルート（`skill` / `workflow` / `proposal`）を計画・判定するインテリジェントプランナーです。

### 主な機能
*   既存スキル（インベントリ）一覧のロードとセマンティックな理解。
*   要件プロンプトの難易度・依存度に応じた3段階開発計画の評価（`skill`, `workflow`, `proposal`）。
...
- 「この機能要件 '...' はどのような開発方針で進めるべきですか？」
- 「機能要件を分析して開発計画を立案してください。」
