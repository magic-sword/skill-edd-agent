...
2. **決定論的スクリプトの実装 (`scripts/`)**:
   ...
   - 軽量ユーティリティは標準ライブラリで完結させますが、高度な業務処理（PyTorch, Pandas, Playwright, openpyxl 等）を標準ライブラリだけで無理に自作（車輪の再発明）することは避けてください。外部パッケージを必要とするスキルは、`SKILL.md` の `## Requirements & Prerequisites` に明記し、`argparse` で `--help` に対応させてください。
