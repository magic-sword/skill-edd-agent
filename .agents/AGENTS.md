...
* **汎用ランタイム＆テストハーネス (`edd-agent-tools` - pip パッケージ)**:
  ...
  ※ 他プロジェクトに `pip install` された環境でも単独で完全動作するよう、パッケージ内部は外部プロジェクト固有パスへの暗黙依存を持たない完全自己完結設計とします。公式 Code Executor および ADK 公式評価器（TrajectoryEvaluator, AgentEvaluator, RubricBasedFinalResponseQualityV1Evaluator, EvalConfig）を使用します。Tool Trajectory 検証およびスクリプト実行は、ADK 2.0 純正の `run_skill_script`（args: `skill_name`, `file_path`, `args`, `positional_args`）および `RunSkillScriptTool` に完全一本化し、ドメイン層での展開コード再実装やエージェントプロンプト内でのスキル名ハードコードを徹底排除します。
